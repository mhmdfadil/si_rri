...
<div class="max-w-full mx-auto">
    ...
    <div class="grid grid-cols-1 md:grid-cols-5 gap-4 mb-6">
        <div class="bg-white dark:bg-gray-800 rounded-xl p-4 shadow-lg border border-gray-100 dark:border-gray-700 transition-colors">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-sm text-gray-600 dark:text-gray-400">Total Konten</p>
                    <p class="text-2xl font-bold text-gray-800 dark:text-gray-100 mt-1">{{ \App\Models\KontenSiaran::count() }}</p>
                </div>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl p-4 shadow-lg border border-gray-100 dark:border-gray-700 transition-colors">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-2xl font-bold text-blue-600 dark:text-blue-400 mt-1">{{ \App\Models\KontenSiaran::hariIni()->count() }}</p>
                </div>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl p-4 shadow-lg border border-gray-100 dark:border-gray-700 transition-colors">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-sm text-gray-600 dark:text-gray-400">Siap Tayang</p>
                    <p class="text-2xl font-bold text-yellow-600 dark:text-yellow-400 mt-1">{{ \App\Models\KontenSiaran::siapTayang()->count() }}</p>
                </div>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl p-4 shadow-lg border border-gray-100 dark:border-gray-700 transition-colors">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-sm text-gray-600 dark:text-gray-400">Sedang Tayang</p>
                    <p class="text-2xl font-bold text-green-600 dark:text-green-400 mt-1">{{ \App\Models\KontenSiaran::tayang()->count() }}</p>
                </div>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl p-4 shadow-lg border border-gray-100 dark:border-gray-700 transition-colors">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-2xl font-bold text-gray-600 dark:text-gray-300 mt-1">{{ \App\Models\KontenSiaran::draft()->count() }}</p>
                </div>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-xl shadow-lg border border-gray-100 dark:border-gray-700 p-6 mb-6 transition-colors">
        <form action="{{ route('konten-siaran.index') }}" method="GET" class="space-y-4">
                ...
                <div class="md:col-span-2">
                    ...
                    <div class="relative">
                        <input 
                            ...
                            class="w-full pl-10 pr-4 py-2.5 border border-gray-300 dark:border-gray-600 rounded-lg focus:border-blue-500 dark:focus:border-blue-400 focus:ring-2 focus:ring-blue-100 dark:focus:ring-blue-900 transition-all outline-none bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100"
                        >
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Quick Filter</label>
                    <select 
                        name="filter_tanggal" 
                        class="w-full px-4 py-2.5 border border-gray-300 dark:border-gray-600 rounded-lg focus:border-blue-500 dark:focus:border-blue-400 focus:ring-2 focus:ring-blue-100 dark:focus:ring-blue-900 transition-all outline-none bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100"
                    >
        ...
        <div class="overflow-x-auto">
            <table class="w-full">
                <thead class="bg-gray-50 dark:bg-gray-700 border-b border-gray-200 dark:border-gray-600">
                    <tr>
                        <th class="px-6 py-4 text-left text-xs font-bold text-gray-700 dark:text-gray-300 uppercase tracking-wider">Konten</th>
                        <th class="px-6 py-4 text-left text-xs font-bold text-gray-700 dark:text-gray-300 uppercase tracking-wider">Program & Kategori</th>
                        <th class="px-6 py-4 text-left text-xs font-bold text-gray-700 dark:text-gray-300 uppercase tracking-wider">Jadwal Tayang</th>
                        <th class="px-6 py-4 text-left text-xs font-bold text-gray-700 dark:text-gray-300 uppercase tracking-wider">Narasumber</th>
                ...
                <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                    ...
                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
                        ...
                        <td class="px-6 py-4">
                            <div class="flex items-start gap-3">
                                ...
                                >
                                <div>
                                    <p class="text-sm font-bold text-gray-800 dark:text-gray-100 line-clamp-2">{{ $item->judul }}</p>
                                    <p class="text-xs text-gray-600 dark:text-gray-400 mt-1">{{ $item->kode_konten }}</p>
                                    <div class="flex items-center gap-2 mt-1">
                                        @if($item->tipe_siaran == 'live')
                                        <span class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium bg-red-50 dark:bg-red-900/30 text-red-700 dark:text-red-300">
                                        ...
                                        @endif
                                        <span class="text-xs text-gray-500 dark:text-gray-400">• {{ $item->durasi_formatted }}</span>
                                    </div>
                                </div>
                            </div>
                        </td>
                        ...
                        <!-- Program & Kategori -->
                        <td class="px-6 py-4">
                            <p class="text-sm font-medium text-gray-800 dark:text-gray-100">{{ $item->program->nama_program ?? '-' }}</p>
                            <p class="text-xs text-gray-600 dark:text-gray-400 mt-1">{{ $item->kategori->nama_kategori ?? '-' }}</p>
                            <span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-purple-50 dark:bg-purple-900/30 text-purple-700 dark:text-purple-300 mt-1">
                        ...
                        <td class="px-6 py-4">
                            <div class="flex items-start gap-2">
                                ...
                                <div>
                                    <p class="text-sm text-gray-800 dark:text-gray-100">{{ $item->tanggal_siaran_formatted }}</p>
                                    <p class="text-xs text-gray-600 dark:text-gray-400 mt-0.5">{{ $item->jam_siaran_formatted }} WIB</p>
                                </div>
                            </div>
                        </td>
                            ...
                            @if($item->narasumbers->count() > 0)
                                <div class="flex -space-x-2">
                                    ...
                                    <div class="w-8 h-8 rounded-full border-2 border-white dark:border-gray-800 bg-gray-200 dark:bg-gray-600 flex items-center justify-center">
                                        <span class="text-xs font-medium text-gray-600 dark:text-gray-300">+{{ $item->narasumbers->count() - 3 }}</span>
                                    </div>
                                    @endif
                                </div>
                                <p class="text-xs text-gray-600 dark:text-gray-400 mt-1">{{ $item->narasumbers->count() }} narasumber</p>
                            @else
                                <p class="text-xs text-gray-500 dark:text-gray-400 italic">Belum ada narasumber</p>
                            @endif
                        </td>
                        ...
                        <td class="px-6 py-4">
                            @php
                                $statusColors = [
                                    'draft' => ['bg' => 'bg-gray-50 dark:bg-gray-700', 'text' => 'text-gray-700 dark:text-gray-300', 'dot' => 'bg-gray-500'],
                                    'diajukan' => ['bg' => 'bg-blue-50 dark:bg-blue-900/30', 'text' => 'text-blue-700 dark:text-blue-300', 'dot' => 'bg-blue-500'],
                                    'disetujui' => ['bg' => 'bg-green-50 dark:bg-green-900/30', 'text' => 'text-green-700 dark:text-green-300', 'dot' => 'bg-green-500'],
                                    'ditolak' => ['bg' => 'bg-red-50 dark:bg-red-900/30', 'text' => 'text-red-700 dark:text-red-300', 'dot' => 'bg-red-500'],
                                    'siap_tayang' => ['bg' => 'bg-yellow-50 dark:bg-yellow-900/30', 'text' => 'text-yellow-700 dark:text-yellow-300', 'dot' => 'bg-yellow-500'],
                                    'tayang' => ['bg' => 'bg-green-50 dark:bg-green-900/30', 'text' => 'text-green-700 dark:text-green-300', 'dot' => 'bg-green-500'],
                                    'selesai' => ['bg' => 'bg-gray-50 dark:bg-gray-700', 'text' => 'text-gray-700 dark:text-gray-300', 'dot' => 'bg-gray-500'],
                                    'dibatalkan' => ['bg' => 'bg-red-50 dark:bg-red-900/30', 'text' => 'text-red-700 dark:text-red-300', 'dot' => 'bg-red-500'],
                                ];
                                $colors = $statusColors[$item->status] ?? $statusColors['draft'];
                            @endphp
                    ...
                    <tr>
                        <td colspan="6" class="px-6 py-12 text-center">
                            ...
                            <p class="text-gray-500 dark:text-gray-400 font-medium">Tidak ada data konten siaran</p>
                            <p class="text-gray-400 dark:text-gray-500 text-sm mt-1">Silakan tambah konten siaran baru</p>
                        </td>
                    </tr>
                    @endforelse
                </tbody>
